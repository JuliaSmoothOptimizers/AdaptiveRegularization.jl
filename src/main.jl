  ...
  while !OK
    ...
    while !success & (unsuccinarow < max_unsuccinarow)
      ...
      if Δq < 0.0 # very unsucessful
        verbose > 0 && mod(stats.iter, verbose) == 0 && @info log_row(Any[iter, ft, norm_∇f, λ, "VU", α, norm(d), Δq])
        unsucc += 1
        unsuccinarow += 1
      ...
      elseif r < acceptance_threshold # unsucessful
        verbose > 0  && mod(stats.iter, verbose) == 0 && @info log_row(Any[iter, ft, norm_∇f, λ, "U", α, norm(d), Δq])
        unsucc += 1
        unsuccinarow += 1
        ...
        if r > increase_threshold # very sucessful
          ...
          verbose > 0 && mod(stats.iter, verbose) == 0 && @info log_row(Any[iter, ft, norm_∇f, λ, "V", α, norm(d), Δq])
        else # sucessful
          if r < reduce_threshold
            α = decrease(PData, α, TR)
          end
          verbose > 0 && mod(stats.iter, verbose) == 0 && @info log_row(Any[iter, ft, norm_∇f, λ, "S", α, norm(d), Δq])
          succ += 1
        end
      end
    end # while !success
