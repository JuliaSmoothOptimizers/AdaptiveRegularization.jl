...
for solver in ["ARC", "TRU"]
  ...
  for pb in problems
    ...
    for line in result
      if occursin("Total time", line)
        push!(time_table, parse(Float64, replace(split(line, "=")[2], "seconds" => "")))
      end
  ...
  df = DataFrame(
    [name_table, time_table, objective_table, iter_table, dual_feas_table, neval_obj_table, neval_grad_table, neval_hprod_table],
    [:name, :elapsed_time, :objective, :iter, :dual_feas, :neval_obj, :neval_grad, :neval_hprod],
  )
