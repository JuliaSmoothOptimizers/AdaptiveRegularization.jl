...
function NLPModels.hprod!(::EmptyNLPModel, x::AbstractVector{T}, v::AbstractVector{T}, Hv::AbstractVector{T}; obj_weight = one(T)) where {T}
    Hv .= zero(T)
    return Hv
end
function NLPModels.hess_structure!(::EmptyNLPModel{T,S}, rows::AbstractVector, cols::AbstractVector) where {T,S}
    rows .= one(T)
    cols .= one(T)
    return rows, cols
end
function NLPModels.hess_coord!(::EmptyNLPModel{T,S}, x::AbstractVector{T}, vals::AbstractVector; obj_weight = one(T)) where {T,S}
    vals .= zero(T)
    return vals
end
...
stp = NLPStopping(nlp)
stp.meta.max_iter = 5
TRnothing = TrustRegion(10., max_unsuccinarow = 2)
